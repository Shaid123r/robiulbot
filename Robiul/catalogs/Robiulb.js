...
const config = require("../../Robiul.json");
...
const logger = require("./Robiulc.js");
...
global.utils = require("./Robiuld.js");
global.loading = require("./Robiulc.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/Robiul.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../Robiul.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('Robiul')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('Robiul')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('Robiul')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('Robiul')} file`, "error")
}
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('Robiul.json')} file`);
    process.exit(0);
  }
  if (!global.config.PREFIX) {
    logger.error(`please enter your bot prefix in ${chalk.blueBright('Robiul.json')} file`)
  }
  if (global.config.OWNER != "61566763566725") {
    logger.error(`detected : OWNER was changed at ${chalk.blueBright('Robiul.json')}`);
    process.exit(0);
  }
  if (packages.author != "Robiul") {
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../Robiulstate.json"));
  ...
  logger.loader(`deployed ${chalk.blueBright('Robiulstate')} file`)
} catch (e) {
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../Robiul.json`);
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/Robiul.json');
    ...
    global.custom = require('../../Robiul.js')({ api: loginApiData });
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "Robiul");
    logger(`deploying ${chalk.blueBright('login')} system`, "Robiul")
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "Robiul") }
})();